...
class Config:
    # Paths
    DATA_DIR = './data'
    MODEL_DIR = './sciq_models_train'
    OUTPUT_DIR = './sciq_multi_task'
    TRAIN_DATA_PATH = f'{DATA_DIR}/sciq_train_converted.json'
    VALID_DATA_PATH = f'{DATA_DIR}/sciq_valid_converted.json'
    TEST_DATA_PATH = f'{DATA_DIR}/sciq_test_1.json'
    
    
    # Model Configuration
    MODEL_NAME = 't5-base'
    MODEL_VERSION = 'sciq_multi_task_005'  # Version number for saving models
...
def load_data():
    ...
    print(f"Total Dataset:     {len(train_df) + len(val_df) + len(test_df):,} examples\n")
...
def prepare_data_for_training(df):
    ...
    for _, row in df.iterrows():
        ...
        # Task 1
        input_texts.append(f"generate_distractors: {sentence} [SEP] {answer}")
        target_texts.append(', '.join(distractors))

        # Task 2
        choices = distractors + [answer]
        np.random.shuffle(choices)
        input_texts.append(f"choose_answer: {sentence} [SEP] {', '.join(choices)}")
        target_texts.append(answer)
        
    print("Number of input examples:", len(input_texts))
    print("Number of target examples:", len(target_texts))
    print("Sample input:", input_texts[1])
    print("Sample target:", target_texts[1])
...
def train_model(train_data, val_data, model_type="ft"):
    ...
    # Write to text file
    with open("sciq_multi_task_005.txt", "w") as file:
...
def generate_distractors(model, tokenizer, sentence, answer):
    ...
    input_text = f"generate_distractors: {sentence} [SEP] {answer}"
    input_ids = tokenizer(
        input_text, 
        return_tensors='pt'
...
def run_pipeline(model_type="ft"):
    ...
    # Print one example from the train_df 'input' column
    #print(train_data['input'].iloc[0])
...
if __name__ == "__main__":
   ...
   parser.add_argument('--model_type', type=str, default='ft', choices=['ft'],
                     help='Model type: fine-tuned (ft)')
