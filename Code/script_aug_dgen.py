...
class Config:
    ...
    OUTPUT_DIR = './dgen_outputs'
    TRAIN_DATA_PATH = f'{DATA_DIR}/dgen_aug_train.json'
    TEST_DATA_PATH = f'{DATA_DIR}/dgen_aug_test.json'
    
    # Model Configuration
    MODEL_NAME = 't5-base'
    MODEL_VERSION = 'dgen_aug_G10_k_10'  # Version number for saving models
...
def load_data():
    ...
    # Add prefixes
    train_df['input'] = 'Question: ' + train_df['sentence'] + ', Answer:  ' + train_df['answer'] + ' [SEP] ' + train_df['prompt_distractors'].apply(lambda x: ', '.join(map(lambda d: str(d).lower(), x[:10])))
    test_df['input'] =  'Question: ' + test_df['sentence'] + ', Answer:  ' + test_df['answer'] 
    #'' + test_df['sentence'] + ' [SEP] ' + test_df['answer'] + ' [SEP] ' + test_df['prompt_distractors'].apply(lambda x: ', '.join(map(str, x)))
